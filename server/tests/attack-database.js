// The step 1 attacks, re-run on every push as a regression gate.
//
...
export async function attackDatabase(record) {
  ...
  await shouldFail(record, 'insert an approved answer', 'must be born DRAFT',
    () => prisma.$executeRaw`
      ...
      VALUES ('atk-forged', ${d}, 'draft', 'APPROVED', 'forged', ARRAY[]::text[], 'none', 'v1')`);

  await shouldFail(record, 'insert skipping straight to review', 'must be born DRAFT',
    () => prisma.$executeRaw`
      INSERT INTO "Answer" (id,"doubtId","aiDraft",status,"riskFlags",model,"promptVersion")
      VALUES ('atk-skip', ${d}, 'draft', 'PENDING_REVIEW', ARRAY[]::text[], 'none', 'v1')`);

  await shouldFail(record, 'insert a draft holding published text', 'check constraint',
    () => prisma.$executeRaw`
      INSERT INTO "Answer" (id,"doubtId","aiDraft",status,"publishedText","riskFlags",model,"promptVersion")
      VALUES ('atk-sneak', ${d}, 'draft', 'DRAFT', 'leaked', ARRAY[]::text[], 'none', 'v1')`);

  await shouldPass(record, 'insert a draft', 'allowed', () => draft('atk-a', d));
  ...
  await shouldPass(record, 'answer the doubt again after rejection', 'allowed', () => draft('atk-b', d));
  ...
  await shouldPass(record, 'a competing draft', 'allowed', () => draft('atk-c', d));
  ...
  await shouldFail(record, 'delete then reinsert as approved', 'must be born DRAFT', async () => {
    ...
    await prisma.$executeRaw`
      INSERT INTO "Answer" (id,"doubtId","aiDraft",status,"publishedText","riskFlags",model,"promptVersion")
      VALUES ('atk-c', ${d}, 'draft', 'APPROVED', 'reinserted', ARRAY[]::text[], 'none', 'v1')`;
  });
